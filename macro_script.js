...
const TELEGRAM_ENTRY_NOTIFY_SESSION_KEY = `quizSiteEntryNotified_${QUIZ_STORAGE_NAMESPACE}_${CURRENT_QUIZ_SECTION || 'macro'}_v1`;
const TELEGRAM_BLOCKED_ENTRY_NOTIFY_SESSION_KEY = `quizBlockedSiteEntryNotified_${QUIZ_STORAGE_NAMESPACE}_${CURRENT_QUIZ_SECTION || 'macro'}_v1`;
...
const QUIZ_SECTION_CONFIGS = {
  macro: {
    ...
    shortLabel: 'Макро',
    homePage: 'index.html',
    testPage: 'macro_test.html',
    defaultThemeFile: 'macro_tests.json',
    supportsTesting: true,
    description: 'Отдельный раздел с тестами по макроэкономике',
    themeFiles: [
      { file: 'macro_001_050.json', label: 'Вопросы 1-50' },
      { file: 'macro_051_100.json', label: 'Вопросы 51-100' },
      { file: 'macro_101_150.json', label: 'Вопросы 101-150' },
      { file: 'macro_151_200.json', label: 'Вопросы 151-200' },
      { file: 'macro_201_250.json', label: 'Вопросы 201-250' },
      { file: 'macro_251_300.json', label: 'Вопросы 251-300' },
      { file: 'macro_301_302.json', label: 'Вопросы 301-302' },
      { file: 'macro_tests.json', label: 'Все вопросы (Микс)', default: true }
    ],
    totalQuestionCount: 302
  }
};

const CURRENT_SECTION_CONFIG = QUIZ_SECTION_CONFIGS[CURRENT_QUIZ_SECTION] || QUIZ_SECTION_CONFIGS.macro;
const PREMIUM_ACCESS_STORAGE_KEY = 'quizPremiumActivationMacro_v1';
const PREMIUM_ACCESS_LEGACY_STORAGE_KEY = 'quizPremiumActivationMacro_legacy_v0';
const PREMIUM_ACTIVATION_PUBLIC_KEY_PEM = `-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEGwqhpdtO85DM1gSiOvaH0F+cVnAE
lj37cd7aXy2sU0sXTHoYhFJzha+CtN7Lx9ntFhUCfV/RQ07fOJj1LEJ6Dw==
-----END PUBLIC KEY-----`;
...
const PREMIUM_ACTIVATION_TOKEN_CONFIGS = {
  MCR1: { version: 'macro-v1', accessType: PREMIUM_ACCESS_TYPES.MACRO }
};
...
const SUBSCRIPTION_PAYMENT_CONFIG = window.SUBSCRIPTION_PAYMENT_CONFIG || {
  ...
  plans: [
    { title: 'Макроэкономика', price: '25 000 сум', oldPrice: '30 000 сум', discountLabel: '−17%', description: 'Доступ только к разделу макроэкономики', accessType: PREMIUM_ACCESS_TYPES.MACRO }
  ]
};
...
const APP_MENU_ITEMS = window.APP_MENU_ITEMS || [
  {
    ...
    label: 'Макроэкономика',
    href: 'index.html',
    description: 'Основной раздел с тестами по макроэкономике',
    requiredSection: 'macro',
    children: []
  },
  {
    type: 'action',
...
const APP_NEWS_ITEMS = window.APP_NEWS_ITEMS || [
  {
    date: '04.05.2026',
    type: 'Добавлено',
    title: 'Отдельный раздел макроэкономики',
    items: [
      'Сайт собран отдельно от других разделов.',
      'Ключи доступа используют отдельный префикс MCR1.',
      'Правильные ответы сверены с итоговым контролем для совпавших вопросов.'
    ]
  }
];
...
function readTemporaryDailyLimitStore() {
  try {
    const parsed = JSON.parse(localStorage.getItem(TEMPORARY_DAILY_TEST_STORAGE_KEY) || '{}');
...
function writeTemporaryDailyLimitStore(store) {
  try {
    localStorage.setItem(TEMPORARY_DAILY_TEST_STORAGE_KEY, JSON.stringify(store && typeof store === 'object' ? store : {}));
  } catch (_) {}
}
...
function consumeTemporaryDailyTestLimit() {
  ...
  if (timestamps.length >= TEMPORARY_DAILY_TEST_LIMIT) {
    store[userKey] = { timestamps };
    writeTemporaryDailyLimitStore(store);
    return { allowed: false, snapshot: normalizeTemporaryDailyLimitSnapshot({
      isPremium: false,
      limit: TEMPORARY_DAILY_TEST_LIMIT,
    ...
    <div class="agreement-panel subscription-panel temporary-limit-panel">
      ...
      <div class="limit-plan-panel" data-limit-panel="temporary">
        ...
        <ul class="limit-benefits-list">
          ...
          <li>Обычный режим тестирования</li>
          <li>История, избранные и изучение вопросов</li>
        </ul>
      </div>
...
function normalizePremiumAccessType(value) {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === 'macro') return PREMIUM_ACCESS_TYPES.MACRO;
  return PREMIUM_ACCESS_TYPES.MACRO;
}
...
function hasSectionAccess(accessType, sectionKey) {
  const targetSection = String(sectionKey || CURRENT_QUIZ_SECTION || 'macro').trim().toLowerCase();
...
function normalizeSectionKey(value, fallback = CURRENT_QUIZ_SECTION || 'macro') {
  const normalized = String(value || fallback || 'macro').trim().toLowerCase();
  return QUIZ_SECTION_CONFIGS[normalized] ? normalized : 'macro';
}
...
function normalizePremiumActivationStore(value) {
  ...
  if (value && typeof value === 'object' && value.users && typeof value.users === 'object') {
    Object.entries(value.users).forEach(([rawUserId, rawRecord]) => {
      ...
      if (!userId) return;
      const accessType = normalizePremiumAccessType(rawRecord?.accessType || rawRecord?.scope || rawRecord?.section || rawRecord?.plan || 'macro');
      users[userId] = {
        ...
        version: String(rawRecord?.version || 'macro-v1'),
...
function hasStoredPremiumAccessForUser(userId, requiredSection = CURRENT_QUIZ_SECTION) {
  ...
  const section = String(requiredSection || CURRENT_QUIZ_SECTION || 'macro').trim().toLowerCase();
...
function grantPremiumAccessForUser(userId, details = {}) {
  ...
  const accessType = normalizePremiumAccessType(details?.accessType || details?.scope || details?.section || 'macro');
  ...
  const mergedRecord = {
    ...
    version: String(details?.version || existingRecord?.version || 'macro-v1'),
...
function accessRecordAllowsSection(record, sectionKey = CURRENT_QUIZ_SECTION) {
  if (!record) return false;
  const section = String(sectionKey || CURRENT_QUIZ_SECTION || 'macro').trim().toLowerCase();
  const sections = Array.isArray(record.sections) && record.sections.length
    ? record.sections.map((item) => String(item || '').trim().toLowerCase()).filter(Boolean)
    : getSectionsForAccessType(record.accessType);
  return sections.includes(section) || hasSectionAccess(record.accessType, section);
...
function getDefaultQuizThemeFile() {
  return CURRENT_SECTION_CONFIG.defaultThemeFile || 'macro_tests.json';
}
...
function renderStudyList() {
  ...
  container.innerHTML = visibleItems.map((entry, visibleIndex) => {
    ...
    const hasGeneratedOptions = item?.chatgptGeneratedOptions === true;
    const generatedBadge = hasGeneratedOptions
      ? '<div class="generated-options-badge generated-options-badge-study">Варианты сгенерированы с помощью ChatGPT; правильный ответ совпадает с итоговым контролем</div>'
      : '';
...
function showMarketingMenuHint() {
  ...
  hint.innerHTML = `
    ...
    <div class="app-menu-hint-text">
      Там есть раздел <b>Макроэкономика</b>, а также <b>Изучить тесты</b>, <b>История</b> и <b>Статистика</b>.
    </div>
...
function showComingSoonNotice(message = 'Скоро') {
  ...
  card.innerHTML = `
    <div style="font-size:17px;font-weight:800;margin-bottom:6px;">Макроэкономика</div>
    <div style="font-size:28px;font-weight:900;line-height:1.1;margin-bottom:8px;">Скоро</div>
    <div style="font-size:14px;line-height:1.4;color:#6d6f82;">Раздел появится позже.</div>
  `;
...
function getAiOptionsNoteHtml() {
  return '<div class="ai-options-note">Варианты ответов сгенерированы с помощью ChatGPT. Правильные ответы сверены с итоговым контролем для совпавших вопросов.</div>';
}
...
function showQuestion() {
  ...
  if (!q) return finish();
  const questionSourceBadgeHtml = q?.chatgptGeneratedOptions === true
    ? '<div class="generated-options-badge">Варианты сгенерированы с помощью ChatGPT; правильный ответ совпадает с итоговым контролем</div>'
    : '';
